...
class VisitantesTable {
    ...
    public function obtenerFiltroDatosVisitantes($start,$length,$search=null,$idferias=null,$idperfil=null,$totalregistro=false){
        ...
        if($idperfil != 1 && $idferias != null)$condiciones[] = "idferias = {$idferias}";
        if($search!=null)$condiciones[] = "nombres LIKE '%".$search."%' OR apellido_paterno LIKE '%".$search."%' OR apellido_materno LIKE '%".$search."%' OR numero_documento LIKE '%".$search."%' OR correo LIKE '%".$search."%'";
    ...
    public function totalUsuariosEfectivos($idferias=null,$fecha=null){
        ...
        if($fecha!=null)$condiciones[] = "fecha_registro_web >= '{$fecha_inicio} 00:00:00' AND fecha_registro_web <= '{$fecha_fin} 23:59:59'";
        if( !empty($condiciones) ) $sql .= " WHERE ".implode(" AND ", $condiciones);
