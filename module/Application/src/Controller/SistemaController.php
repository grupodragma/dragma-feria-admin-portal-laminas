...
class SistemaController extends AbstractActionController {
    ...
    public function editarUsuarioAction(){
        ...
        if(file_exists(getcwd().'/public'.$this->dir_avatar.$dataUsuario['token'].'.jpg'))$dataUsuario['img_user_perfil'] = $this->dir_avatar.$dataUsuario['token'].'.jpg';
        else $dataUsuario['img_user_perfil'] = $this->dir_avatar.'male.png';
    ...
    public function validarCorreoAction(){
        ...
        else echo '"Este correo electrónico ya se encuentra en uso"';
        die;
    }
    ...
    public function validarUsuarioAction(){
        ...
        if($result == false || $usuario == $key) echo '"true"';
        else echo '"Este usuario ya se encuentra en uso"';
        die;
    }
    ...
    public function listarPerfilesAction(){
        ...
        foreach($perfiles as $perfil){
            ...
            foreach($perfil['modulos'] as $modulo){
                if(isset($modulo['icono']))$modulos[] = '<i class="'.$modulo['icono'].'"></i> '.$modulo['nombre'];
            }
            ...
            if($perfil['idperfil'] == 1)$blocked = 'disabled';
            else $blocked = '';
    ...
    public function editarPerfilAction(){
        ...
        $submodulos = [];
        foreach($dataPerfil['dataModulos'] as $modulo)if(!in_array($modulo['idsubmodulo'],$submodulos))$submodulos[] = $modulo['idsubmodulo'];
    ...
    public function guardarEditarPerfilAction(){
        ...
        $data_submodulos = $this->params()->fromPost('submodulos');
        if(empty($data_submodulos)) $data_submodulos = [];
    ...
    public function guardarAgregarModuloAction(){
        ...
        $data = [
            ...
            'col_grupo_dash'=>1,
            'pos_grupo_dash'=>(int)$this->objModulosTable->obtenerUltimoOrdenModulo()
        ];
        ...
        if(!empty($dataSubModulos) && $lastIdModulo > 0){
            foreach($dataSubModulos as $item){
                ...
                $data = [
                    ...
                    'orden'=>(int)$this->objSubModulosTable->obtenerUltimoOrdenSubModuloPorIdModulo($lastIdModulo)
                ];
        ...
        if(!empty($dataSubModulos) && $idmodulos > 0){
            foreach($dataSubModulos as $item){
                ...
                $data = [
                    ...
                    'visible_grupo_dash'=> 1,
                    'desc_grupo_dash'=> $item['descripcion'],
                    'orden'=>(int)$this->objSubModulosTable->obtenerUltimoOrdenSubModuloPorIdModulo($idmodulos)
                ];
                ...
                if(isset($item['idsubmodulo'])) $idSubModuloSeleccionados[] = $item['idsubmodulo'];
