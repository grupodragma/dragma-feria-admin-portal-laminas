...
class ProductoController extends AbstractActionController {
    ...
    public function __construct($serviceManager, $objProductosTable, $objEmpresasTable, $objProductosImagenesTable, $objDistritosTable, $objTipoHabitacionTable, $objNumeroHabitacionTable, $objRangoPreciosTable, $objEtapaTable, $objTools, $objBancosTable) {
    ...
    public function listarProductosAction(){
        $dataProductos = $this->objProductosTable->obtenerProductos($this->sessionContainer['idferias'], $this->sessionContainer['idperfil']);
        $data_out = [];
        $data_out['data'] = [];
    ...
    public function agregarProductosAction(){
        $data=[
            'empresas'=> $this->objEmpresasTable->obtenerEmpresas($this->sessionContainer['idferias'], $this->sessionContainer['idperfil']),
    ...
    public function guardarAgregarProductosAction(){
        ...
        $data = [
            ...
            'iddistritos'=>(isset($datosFormulario['iddistritos']) && !empty($datosFormulario['iddistritos']))?$datosFormulario['iddistritos']:0,
            'destacado'=>(isset($datosFormulario['destacado']))?$datosFormulario['destacado']:0,
            'precio_especial'=>!empty($datosFormulario['precio_especial']) ? $datosFormulario['precio_especial'] : 0,
            'idtipohabitacion'=>(int)$datosFormulario['idtipohabitacion'],
            'idnumerohabitacion'=>(int)$datosFormulario['idnumerohabitacion'],
            'idetapa'=>(int)$datosFormulario['idetapa'],
            'ubicacion'=>@$datosFormulario['inputSearch'],
            'latitud'=>$datosFormulario['latitud'],
            'longitud'=>$datosFormulario['longitud'],
            'sala_venta'=>(isset($datosFormulario['sala_venta']))?$datosFormulario['sala_venta']:0,
            'sala_venta_rv'=>$datosFormulario['sala_venta_rv'],
            'hash_url'=>$this->objTools->toAscii($datosFormulario['nombre']),
            'titulo_recuadro'=>$datosFormulario['titulo_recuadro'],
            'recorrido_virtual_360'=>$datosFormulario['recorrido_virtual_360'],
            'idbancos'=>$datosFormulario['idbancos'],
        ];
            ...
            foreach($archivos as $key => $archivo){
                ...
                switch($key){
                    ...
                    case 'hash_ubicacion_mapa':
                        $imagenExtensionesValidas = $this->imagenExtensionesValidas;
                        $carpetaProductoArchivos = getcwd().'/public/productos/imagen';
                        $keyDataArchivo[$key] = ['hash'=> 'hash_ubicacion_mapa', 'nombre'=> 'nombre_ubicacion_mapa'];
                    break;
                    case 'hash_reel_video':
    ...
    public function editarProductosAction(){
        ...
        $dataProductos['empresas'] = $this->objEmpresasTable->obtenerEmpresas($this->sessionContainer['idferias'], $this->sessionContainer['idperfil']);
        $dataProductos['imagenes'] = $this->objProductosImagenesTable->obtenerDatosProductosImagenes(['idproductos'=> $idproductos]);
        $dataProductos['distritos'] = $this->objDistritosTable->obtenerDistritos();
        $dataProductos['tipoHabitaciones'] = $this->objTipoHabitacionTable->obtenerTipoHabitacion();
        $dataProductos['numeroHabitaciones'] = $this->objNumeroHabitacionTable->obtenerNumeroHabitacion();
        $dataProductos['etapas'] = $this->objEtapaTable->obtenerEtapa();
        $dataProductos['bancos'] = $this->objBancosTable->obtenerBancos();
        ...
        if(!empty($archivos) && isset($archivos['imagenes'])){
            foreach($archivos['imagenes'] as $key => $archivo){
                ...
                if( !empty($archivo['name']) && $archivo['size'] !== 0 ) {
                    ...
                    if($datosFormulario['idproductosimagenes'][$i] != ''){
                        $this->objProductosImagenesTable->actualizarDatosProductosImagenes($dataImagenes, $datosFormulario['idproductosimagenes'][$i]);
                    } else if($datosFormulario['idproductosimagenes'][$i] == '' && $datosFormulario['titulo'][$i] != '') {
                        $dataImagenes['idproductos'] = $idproductos;
                        $this->objProductosImagenesTable->agregarProductosImagenes($dataImagenes);
                    }
                }
    ...
    public function confirmarEliminarProductosAction(){
        ...
        if( $dataProducto ) {
            $dataArchivos = [
                'archivo_pdf'=> ['directorio'=> getcwd().'/public/productos/documentos'],
                'imagen'=> ['directorio'=> getcwd().'/public/productos/imagen'],
                'super_oferta'=> ['directorio'=> getcwd().'/public/productos/imagen'],
                'ubicacion_mapa'=> ['directorio'=> getcwd().'/public/productos/imagen'],
                'reel_video'=> ['directorio'=> getcwd().'/public/productos/video'],
            ];
